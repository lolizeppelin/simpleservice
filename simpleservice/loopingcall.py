...
class DynamicLoopingCall(LoopingCallBase):
    ...
    _RUN_ONLY_ONE_MESSAGE = "A dynamic interval looping call can only run one function at a time"
    _TASK_MISSING_SLEEP_VALUE_MESSAGE = "A dynamic interval looping call should " \
                                        "supply either an interval or periodic_interval_max"
        ...
        def _idle_for(success, _elapsed):
            ...
            if success:
                ...
                if timeout > 0 and self._error_time + idle > timeout:
                    raise LoopingCallTimeOut('Looping call timed out after %.02f seconds'
                                             % self._error_time)
