...
def _foreign_key_error(integrity_error, match, engine_name, is_disconnect):
    ...
    raise exceptions.DBReferenceError(table, constraint, key, key_table,
                                     integrity_error)
...
def _db2_dupe_key_error(integrity_error, match, engine_name, is_disconnect):
    ...
    raise exceptions.DBDuplicateEntry([], integrity_error)
...
def _raise_for_remaining_DBAPIError(error, match, engine_name, is_disconnect):
    ...
    if is_disconnect:
        ...
        LOG.exception('DBAPIError exception wrapped from %s' % error)
...
def register_engine(engine):
    sqlalchemy.event.listen(engine, "handle_error", handler)
